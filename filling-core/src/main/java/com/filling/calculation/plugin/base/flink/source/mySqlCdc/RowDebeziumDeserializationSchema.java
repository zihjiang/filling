package com.filling.calculation.plugin.base.flink.source.mySqlCdc;

/**
 * @Author zihjiang
 * @Date 2021/12/4 22:15
 * @Version 1.0
 */
 ...
 * Row}.
 */
public final class RowDebeziumDeserializationSchema
    ...
    /**
     * Runtime converter that converts Kafka {@link SourceRecord}s into {@link Row} consisted of
    ...
    /** Returns a builder to build {@link RowDebeziumDeserializationSchema}. */
    ...
    public void deserialize(SourceRecord record, Collector<Row> out) throws Exception {
        ...
        if (op == Envelope.Operation.CREATE || op == Envelope.Operation.READ) {
            Row insert = extractAfterRow(value, valueSchema);
            validator.validate(insert, RowKind.INSERT);
            insert.setKind(RowKind.INSERT);
            emit(record, insert, out);
        } else if (op == Envelope.Operation.DELETE) {
            Row delete = extractBeforeRow(value, valueSchema);
            ...
            Row after = extractAfterRow(value, valueSchema);
    ...
    private Row extractAfterRow(Struct value, Schema valueSchema) throws Exception {
        Schema afterSchema = valueSchema.field(Envelope.FieldName.AFTER).schema();
        Struct after = value.getStruct(Envelope.FieldName.AFTER);
        return (Row) physicalConverter.convert(after, afterSchema);
    }

    private Row extractBeforeRow(Struct value, Schema valueSchema) throws Exception {
        Schema beforeSchema = valueSchema.field(Envelope.FieldName.BEFORE).schema();
        Struct before = value.getStruct(Envelope.FieldName.BEFORE);
        return (Row) physicalConverter.convert(before, beforeSchema);
    }

    private void emit(SourceRecord inRecord, Row physicalRow, Collector<Row> collector) {
        if (!hasMetadata) {
...
//        collector(physicalRow);

        System.out.println("123123123131231312");
    }
    ...
    /** Builder of {@link RowDebeziumDeserializationSchema}. */
    public static class Builder {
        private RowType physicalRowType;
        private TypeInformation<Row> resultTypeInfo;
        ...
        public Builder setResultTypeInfo(TypeInformation<Row> resultTypeInfo) {
        ...
        public RowDebeziumDeserializationSchema build() {
